...
kernel void boundingBox(const device Vector2 *array [[ buffer(0) ]],
    ...
    uint i = bid * blockDim + tid;
    shared_memory[tid] = i;
    shared_memory[tid + blockDim] = i;
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (uint stride = 1; stride < blockDim; stride *= 2) {
        // only accept increasing multiples of two
        if (tid % (2 * stride) == 0) {
            if ( array[shared_memory[tid + stride]].x < array[shared_memory[tid]].x) {
                shared_memory[tid] = shared_memory[tid + stride];
            }
            if ( array[shared_memory[tid + stride + blockDim]].y < array[shared_memory[tid + blockDim]].y) {
                shared_memory[tid + blockDim] = shared_memory[tid + stride + blockDim];
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (0 == tid ) {
        ...
        if( currentXMin == -1 || array[shared_memory[tid]].x < array[currentXMin].x) {
            while(!atomic_compare_exchange_weak_explicit(&result[0], &currentXMin, shared_memory[tid], memory_order_relaxed, memory_order_relaxed)) {
                ...
                if (array[shared_memory[tid]].x > array[currentXMin].x) {
                    break;
                }
            }
        }
        ...
        if( currentYMin == -1 ||  array[shared_memory[tid + blockDim]].y < array[currentYMin].y) {
            while(!atomic_compare_exchange_weak_explicit(&result[1], &currentYMin, shared_memory[tid + blockDim], memory_order_relaxed, memory_order_relaxed)) {
                // another thread won. Check if we still need to set it.
                if (array[shared_memory[tid + blockDim]].y > array[currentYMin].y) {
                    break;
                }
            }
        }
        ...
        if(currentXMaxIndex == -1 || array[shared_memory[tid]].x > array[currentXMaxIndex].x) {
            while(!atomic_compare_exchange_weak_explicit(&result[2], &currentXMaxIndex, shared_memory[tid], memory_order_relaxed, memory_order_relaxed)) {
                // another thread won. Check if we still need to set it.
                if (array[shared_memory[tid]].x < array[currentXMaxIndex].x) {
                    break;
                }
            }
        }
        ...
        if(currentYMax == -1 || array[shared_memory[tid + blockDim]].y > array[currentYMax].y) {
            while(!atomic_compare_exchange_weak_explicit(&result[3], &currentYMax, shared_memory[tid + blockDim], memory_order_relaxed, memory_order_relaxed)) {
                // another thread won. Check if we still need to set it.
                if (array[shared_memory[tid + blockDim]].y < array[currentYMax].y) {
                    break;
                }
            }
        }
